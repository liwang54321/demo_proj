...
awaitable<void> proxy(tcp::socket client, tcp::endpoint target)
{
    ...
    co_await (
        produce_tokens(bytes_per_token, token_interval, client_tokens) && transfer(client, server, client_tokens) && produce_tokens(bytes_per_token, token_interval, server_tokens) && transfer(server, client, server_tokens));
}
...
int main(int argc, char* argv[])
{
    try {
        ...
        auto listen_endpoint = *tcp::resolver(ctx).resolve(argv[1], argv[2],
